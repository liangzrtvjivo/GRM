...
# def evaluate(gold_labels,obs_labels):
#     obs_labels_flat = np.argmax(obs_labels, axis=2).flatten()
#     gold_labels_flat = gold_labels.flatten()
#     total_correct = 0
#     total_count = 0
#     for gold, out in zip(gold_labels_flat,obs_labels_flat):
#         if gold == out:
#             total_correct += 1
#     total_count += len(gold_labels_flat)
#     return total_correct,total_count

def evaluate(gold_labels,obs_labels,num_labels):
    accuracy = metrics.accuracy_score(gold_labels, obs_labels)
    recall = metrics.recall_score(gold_labels, obs_labels, labels=[idx for idx in range(num_labels)],average="macro")
    f1 = metrics.f1_score(gold_labels, obs_labels, labels=[idx for idx in range(num_labels)],average="macro")

...
def rec(corr, gold, obs):
    if gold <= 0 or obs <= 0 or corr <= 0:
        return 0
    rec = corr / gold
    return rec

def pre(corr, gold, obs):
    if gold <= 0 or obs <= 0 or corr <= 0:
        return 0
    pre = corr / obs
    return pre
...
class Evaluator(object):
    ...
    def precision(self, att = None):
        if att != None:
            return pre(self.correct.get(att, 0), self.gold.get(att, 0), self.observed.get(att, 0))
        return pre(sum(self.correct.values()), sum(self.gold.values()), sum(self.observed.values()))

    def recall(self, att = None):
            if att != None:
                return rec(self.correct.get(att, 0), self.gold.get(att, 0), self.observed.get(att, 0))
            return rec(sum(self.correct.values()), sum(self.gold.values()), sum(self.observed.values()))
